...
export class ChatAgent {
    ...
    private documentVisibilityChange = () => {
        document.addEventListener('visibilitychange', async () => {
            if(document.visibilityState === VisibilityStateEnum.VISIBLE) {
                await sleep(2000)
                this.releaseSendChatMutex()
                this.releaseChatMutex()
            ...
            if (chatUserElements.length > 0) {
                for (let i = 0; i < chatUserElements.length; i++) {
                    ...
                    await this.chatMutex.acquire().then(async () => {
                        ...
                        await clickElementHighlight(chatUserElement?.querySelector('.__im_pro__has-contact-action'), 2, 1, false);
                        // 一个候选人格子的高度是62
                        // this.scrollIntoView()
                    });
                }
    ...
    private async handleMutation(mutations: MutationRecord[]) {
        ...
        try {
            ...
            if ( bool && isLastMessageSendByUser) {
                ...
                console.log('处理消息锁--this.sendChatMutex是否lock：：%s', this.sendChatMutex.isLocked())
                await this.sendChatMutex.acquire().then(async () => {
                            ...
                            await sleep(1000)
                            console.log('处理消息锁--this.sendChatMutex--catch--lock：：%s', this.sendChatMutex.isLocked())
                            this.releaseSendChatMutex()
                            if (e !== E_CANCELED) {
    ...
    private async handleMessage(conversationId: string) {
        ...
        console.log('开始处理消息：：是否是我主动发起沟通的：：%s', isFirst)
        if(isFirst) {
            await this.handleFirstSendByMyself(conversationId)
        } else {
            console.log('开始处理消息：：候选人主动发起沟通')
            // 候选人主动发消息
    ...
    private async handleFirstSendByMyself(conversationId:string) {
        ...
        console.log('开始处理消息：：最后一条消息是我回复的吗：：%s', isLastSendBySelf)
        if(isLastSendBySelf) {
            await sleep(1000)
            this.releaseSendChatMutex()
            await sleep(1000)
    ...
    releaseChatMutex () {
        console.log('点击候选人之前是否lock', this.chatMutex.isLocked())
        if(this.chatMutex.isLocked()) {
            this.chatMutex.release();
        }
    }
    releaseSendChatMutex() {
        console.log('点击候选人之前是否lock--releaseSendChatMutex', this.sendChatMutex.isLocked())
        if(this.sendChatMutex.isLocked()) {
            this.sendChatMutex.release()
            console.log('点击候选人之前是否lock--releaseSendChatMutex--释放锁', this.sendChatMutex.isLocked())
        }
    }
}
